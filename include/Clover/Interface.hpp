...
SystemAudioIoConfig GetSystemAudioIoConfig() {
  ...
  systemConfig.defaultInputDeviceIndex = Pa_GetDefaultInputDevice();
  systemConfig.defaultOutputDeviceIndex = Pa_GetDefaultOutputDevice();
...
private:
  static int paCallback(
    ...
    for (unsigned long i = 0; i < framesPerBuffer; i++) {
      ...
      Clover::AudioCallbackStatus status = instance.audioCallback({
  ...
  Clover::AudioCallback audioCallback =
      [](Clover::AudioCallbackArguments args) -> Clover::AudioCallbackStatus {
    throw Exception(
